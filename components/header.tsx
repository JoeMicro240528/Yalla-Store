  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "bg-black/95 backdrop-blur-md opacity-[0.9] shadow-lg shadow-black/20"
          : "bg-transparent"
      )}
    >
      <div className="container mx-auto px-4 flex h-16 items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div className="flex flex-col leading-none">
            <span className="text-sm font-bold text-white tracking-wide">QSERVE</span>
            <span className="text-[10px] text-white uppercase tracking-widest">Yalla Store</span>
          </div>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-1 text-white">
          ...
          <div className="relative" onMouseEnter={() => setProductsOpen(true)} onMouseLeave={() => setProductsOpen(false)}>
            ...
            {productsOpen && (
              <div className="absolute top-full right-0 mt-2 w-[580px] bg-black border border-nav-border rounded-xl shadow-2xl shadow-black/40 p-4 grid grid-cols-2 gap-1">
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          <SheetTrigger asChild className="lg:hidden">
            <Button variant="ghost" size="icon" className="text-white cursor-pointer hover:bg-white/10">
              <Menu className="h-5 w-5" />
              <span className="sr-only">فتح القائمة</span>
            </Button>
          </SheetTrigger>
          <SheetContent side="right" className="w-[300px] bg-black text-white border-0 p-0">
